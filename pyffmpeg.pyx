...
cdef extern from "libavutil/avutil.h":
    ...
    # this is defined below as variable. Originally was a macro
...
#        AVCodecContext *owner                #< the AVCodecContext which ff_thread_get_buffer() was last called on
    ...
    struct AVCodecContext:
        ...
        #EMA int         hurry_up               #< hurry up amount: decoding: Set by user. 1-> Skip B-frames, 2-> Skip IDCT/dequant too, 5-> Skip everything except header
        AVCodec *   codec
        void *      priv_data
...
cdef extern from "libavformat/avformat.h":
    ...
    #int av_open_input_stream(AVFormatContext **ic_ptr,                         AVIOContext *pb, char *filename,                         AVInputFormat *fmt, AVFormatParameters *ap)
...
cdef class Track:
    ...
    def init(self,observer=None, support_truncated=0,   **args):
        ...
        avcodec_open2(self.CodecCtx, self.Codec,NULL)
    ...
    def _reopencodec(self):
        ...
        if (self.Codec.capabilities & CODEC_CAP_TRUNCATED) and (self.support_truncated!=0):
            self.CodecCtx.flags = self.CodecCtx.flags | CODEC_FLAG_TRUNCATED
        ret = avcodec_open2(self.CodecCtx, self.Codec,NULL)
...
cdef class AudioTrack(Track):
    ...
    def reset_tps(self,tps):
        ...
        if self.tps == 0:
            print("pyffmpeg reset_tps: fps is 0")
        else:
            self.audioq=AudioQueue(limitsz=12,tps=self.tps,
                                  samplerate=self.CodecCtx.sample_rate,
                                  destframesize=self.dest_frame_size if (self.dest_frame_size!=0) else (self.CodecCtx.sample_rate//self.tps),
...
cdef class FFMpegReader(AFFMpegReader):
    ...
    def dump(self):
        pass #av_dump_format(self.FormatCtx,0,self.filename,0)
...
#            self.__finalize_open_write()
    ...
    def open(self,char *filename,track_selector=None,mode="r",buf_size=1024):
        ...
        if False:
            if (fmt==NULL) or (not (fmt.flags & AVFMT_NOFILE)):
                ...
                if (buf_size>0):
                    #EMA url_setbufsize(self.FormatCtx.pb,buf_size)
                    pass
        ...
        if (mode=="r"):
            ret =  avformat_open_input(&self.FormatCtx,filename,NULL,NULL)
            #EMA ret = av_open_input_stream(&self.FormatCtx,self.FormatCtx.pb,filename,self.FormatCtx.iformat,NULL)
            if ret != 0:
    ...
    def __finalize_open(self, track_selector=None):
        ...
        try:
            if (self.tracks[0].duration()<0):
                ...
                new_duration=-1 #-self.tracks[0].duration()
    ...
    def duration(self):
        if (self.FormatCtx.duration==0x8000000000000000 or self.FormatCtx.duration < 0):
            raise KeyError
